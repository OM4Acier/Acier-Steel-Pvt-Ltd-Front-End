...
const App: React.FC = () => {
    ...
    const fetchUserAccess = useCallback(async () => {
        try {
            const data = await leadsCenterApi.getUserAccess();
            setUserAccess(data);


            const initialSource = data.allowedSources.includes('combined')
                ? 'combined'
                : (data.allowedSources[0] || '');
    ...
    const fetchLeads = useCallback(async (page: number = 1, currentFilters = filters, signal?: AbortSignal) => {
        ...
        try {
            const data = await leadsCenterApi.getLeadsCenterLeads({
                page,
                source: currentFilters.source,
                date: currentFilters.date,
                search: currentFilters.search,
            ...
            if (data && data.leads) {
                ...
                setTotalRawLeads(data.pagination.totalRecords);
            ...
            if (error.message === 'Request cancelled' || error.name === 'AbortError' || error.message === 'canceled') {
    ...
    }, [clerkUser, fetchLeads, filters, currentPage]);


    const handleReviewAndClaim = useCallback(async (lead: Lead) => {
        setClaimingLeadId(lead.id);
        try {
            const data = await leadsCenterApi.getLeadsCenterDetail(lead.id, filters.source);
            setLeadToEdit(data);
    ...
    const handleClaimSubmit = async (formData: any, files: File[]) => {
        ...
        try {
            ...
            const result = await leadsCenterApi.claimLead(finalPayload);
    ...
    const handleViewDetails = useCallback(async (lead: Lead) => {
        setIsLoadingLeads(true);
        try {
            const data = await leadsCenterApi.getLeadsCenterDetail(lead.id, filters.source);
            setLeadToEdit(data);
    ...
    }, [fetchUserAccess, clerkUser]);


    // Fetch leads only if user profile is available
    useEffect(() => {
        if (clerkUser && userAccess) { // It's good practice to check all dependencies
    ...
    }, [filters.source, filters.date, userAccess, fetchLeads, clerkUser]);
        ...
        <>
            ...
            <div className="min-h-screen flex flex-col items-center p-4 sm:p-8 ">
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-center items-center flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mt-10 p-4 bg-white rounded-xl shadow-md border border-gray-200">
                            <Button
                                onClick={() => {
                                    if (clerkUser) {
                                        fetchLeads(currentPage - 1, filters);
                                    }
                                }}
                                // Disable the button if there's no token
                                disabled={!clerkUser || currentPage === 1 || isLoadingLeads}
                ...
                <LeadFormDialog
                    ...
                    onSubmit={(data, files) => {
                        if (!clerkUser) return; // safely exit if token missing
                        handleClaimSubmit(data, files);
                    }}
                    isSaving={isSaving}

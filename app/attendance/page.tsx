...
export default function AttendancePage() {
  ...
  const fetchConfigAndStatus = useCallback(async () => {
    ...
    // Reset system communication state
    setSystemMessage(null);
    setIsError(false);
    
    try {
      const config = await attendanceApi.getConfig();
      if (config?.locations) setLocations(config.locations);

      try {
        const currentStatus = await attendanceApi.getStatus();
      ...
      setSystemMessage(`Uplink interruption: ${error.message || "Could not sync configurations."}${statusSuffix}`);
      setIsError(true);
  ...
  }, [clerkLoaded, clerkUser, fetchConfigAndStatus]);
  ...
  const handleAttendance = async (type: 'login' | 'logout') => {
    ...
    try {
      ...
      const result = type === 'login'
        ? await attendanceApi.login(record)
        : await attendanceApi.logout(record);
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] dark:bg-[#0a0a0b] text-slate-900 dark:text-slate-100">
        ...
        {/*
              Pattern A — Hanging board.
              <NavbarExtension> renders null but syncs its children into the
              NavbarExtensionSlot inside the NavBar every time this component renders.
            ...
            */}
            ...
            <NavbarExtension>
              <div className="flex items-center gap-2 flex-wrap pb-2">
                ...
                >
                  {isRefreshingStatus ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}

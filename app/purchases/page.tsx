...
import {
  User as UserIcon, Info, CheckCircle, XCircle, Clock, Truck, Plus, Edit, Trash2,
...
type PurchaseStatus = 'Pending' | 'Approved' | 'Cancelled' | 'Invoiced' | 'Completed';
...
type UserRole = 'super-admin' | 'purchase-entry' | 'operations' | 'accountant';
...
interface UserProfile {
  ...
  contactNo?: string;
  organization?: string;
  accessToken: string; // Token received from backend
}
...
const apiService = {
  ...
  loginUser: async (email: string, password: string) => {
    return apiService.authFetch(`${BASE_API_URL}/login`, {
      method: 'POST',
  ...
  // Purchase Management
  fetchPurchases: async (accessToken: string): Promise<Purchase[]> => {
    try {
      const purchases = await apiService.authFetch(`${BASE_API_URL}/purchases`, {
        method: 'GET',
      }, accessToken);
      // Map backend's _id to frontend's id
      return purchases.map((purchase: any) => ({ ...purchase, id: purchase._id }));
    } catch (error) {
      // console.error("API Service - Error fetching purchases:", error);
      throw error;
    }
  },

  addPurchase: async (purchaseData: Omit<Purchase, 'id' | 'createdAt' | 'updatedAt' | 'editHistory' | 'status' | 'unloadingPhotos' | 'cancellationRemarks' | 'vehicleNumber' | 'invoiceDate' | 'invoiceNumber' | 'invoiceFiles' | 'attachments'>, currentUser: UserProfile, accessToken: string): Promise<Purchase> => {
    try {
      const newPurchase = await apiService.authFetch(`${BASE_API_URL}/purchases`, {
        method: 'POST',
        body: JSON.stringify({
          ...purchaseData,
          // Backend will handle id, createdAt, updatedAt, status, editHistory
          createdBy: currentUser.email, // Assuming backend expects this
        }),
      }, accessToken);
      return { ...newPurchase, id: newPurchase._id };
    } catch (error) {
      // console.error("API Service - Error adding purchase:", error);
      throw error;
    }
  },

  // Modified updatePurchase to conditionally add editHistory
  updatePurchase: async (purchaseId: string, updatedFields: Partial<Purchase>, currentUser: UserProfile, description?: string | null, accessToken?: string): Promise<Purchase> => {
    try {
      const payload: any = { ...updatedFields };

      if (description) { // Only add editHistory if description is provided
        const newHistoryEntry: EditHistoryEntry = {
          timestamp: Date.now(),
          editorName: currentUser.name || currentUser.email,
          description: description,
        };
        payload.editHistory = newHistoryEntry; // Backend should append this
      }

      const updatedPurchase = await apiService.authFetch(`${BASE_API_URL}/purchases/${purchaseId}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      }, accessToken);
      return { ...updatedPurchase, id: updatedPurchase._id };
    } catch (error) {
      // console.error("API Service - Error updating purchase:", error);
      throw error;
    }
  },

  deletePurchase: async (purchaseId: string, accessToken: string): Promise<void> => {
    try {
      await apiService.authFetch(`${BASE_API_URL}/purchases/${purchaseId}`, {
        method: 'DELETE',
      }, accessToken);
    } catch (error) {
      // console.error("API Service - Error deleting purchase:", error);
      throw error;
    }
  },

  // File upload/delete using the provided documentation
  // This version assumes the backend returns { success: true, uploadedFiles: [...] }
  uploadFile: async (purchaseNo: string, uploadStage: 'purchase-product' | 'purchase-vehicle' | 'purchase-invoice', files: { filename: string; mimeType: string; fileBase64: string }[], accessToken: string): Promise<{ success: boolean; uploadedFiles: FileReference[] }> => {
    try {
      const response = await apiService.authFetch(`${BASE_API_URL}/files/upload`, {
        method: 'POST',
        body: JSON.stringify({ purchaseNo, uploadStage, files }),
      }, accessToken);
      return response; // Returns { success: true, uploadedFiles: [...] }
    } catch (error) {
  ...
  // This version assumes the backend returns { success: true, message: "File deleted" }
  deleteFile: async (fileId: string, accessToken: string): Promise<{ success: boolean; message: string }> => {
    try {
      const response = await apiService.authFetch(`${BASE_API_URL}/files/delete`, {
        method: 'POST', // Documentation says POST for delete
        body: JSON.stringify({ fileId }),
      }, accessToken);
      return response; // Returns { success: true, message: "File deleted" }
    } catch (error) {
      // console.error("API Service - Error deleting file:", error);
      throw error;
    }
  },

  fetchRecentPurchaseNumber: async (accessToken: string): Promise<{ recentPurchaseNo: string; nextPurchaseNo: string }> => {
    try {
      const response = await apiService.authFetch(`${BASE_API_URL}/purchases/recent-number`, {
        method: 'GET',
      }, accessToken);
      return response;
    } catch (error) {
      // console.error("API Service - Error fetching recent purchase number:", error);
      throw error;
    }
  },
};
...
const PurchaseManagementPanel: React.FC<PurchaseManagementPanelProps> = ({
  ...
  const handleDeleteUploadedFile = async (fileId: string, stage: 'attachments' | 'unloadingPhotos' | 'invoiceFiles') => {
    if (!selectedPurchase || !currentUser || !currentUser.accessToken) {
    ...
    try {
      const response = await apiService.deleteFile(fileId, currentUser.accessToken);
      if (response.success) {
  ...
  const handleAddNewPurchase = async () => {
    ...
    try {
      ...
      if (newPurchaseAttachments.length > 0) {
        ...
        const uploadResponse = await apiService.uploadFile(createdPurchase.purchaseNo, 'purchase-product', filesDataForApi, currentUser.accessToken);
  ...
  const refreshPurchaseNumber = useCallback(async () => {
    ...
    try {
      const { recentPurchaseNo: fetchedRecent, nextPurchaseNo: fetchedNext } =
        await apiService.fetchRecentPurchaseNumber(currentUser.accessToken);
  ...
  const handleOpenNewPurchase = useCallback(() => {
    // 1. Reset all states
    setNewPurchaseData({
      purchaseNo: '',
      supplierName: '',
    ...
    setCurrentUnloadingPhotos([]);
    setCurrentInvoiceFiles([]);
    setEditedPurchaseData({});
    ...
    // 3. Trigger the fetch logic defined above
  ...
  const handleApprovePurchase = async (purchase: Purchase) => {
    ...
    if (!currentUser || !isSuperAdmin(currentUser.role) || !currentUser.accessToken) {
    ...
    try {
      await apiService.updatePurchase(
        ...
        currentUser,
        `Purchase approved by ${currentUser.name || currentUser.email}.`, // Description for history
        currentUser.accessToken
      );
  ...
  const handleCancelPurchaseSubmit = async () => {
    // SECURITY NOTE: Client-side role checks are for UI/UX only. Server-side authorization is crucial for security.
    if (!selectedPurchase || !currentUser || !isSuperAdmin(currentUser.role) || !currentUser.accessToken) {
    ...
    try {
      await apiService.updatePurchase(
        ...
        currentUser,
        `Purchase cancelled by ${currentUser.name || currentUser.email}. Remarks: ${cancelRemarks || 'N/A'}`,
        currentUser.accessToken
      );
  ...
  const handleSaveEditedPurchase = async () => {
    ...
    try {
      ...
      if (selectedAttachments.length > 0) {
        ...
        const uploadResponse = await apiService.uploadFile(selectedPurchase.purchaseNo, 'purchase-product', filesDataForApi, currentUser.accessToken);
      ...
      if (selectedUnloadingPhotos.length > 0) {
        ...
        const uploadResponse = await apiService.uploadFile(selectedPurchase.purchaseNo, 'purchase-vehicle', filesDataForApi, currentUser.accessToken);
      ...
      if (selectedInvoiceFiles.length > 0) {
        ...
        const uploadResponse = await apiService.uploadFile(selectedPurchase.purchaseNo, 'purchase-invoice', filesDataForApi, currentUser.accessToken);
  ...
  const handleTransitionToCompleted = async () => {
    ...
    try {
      await apiService.updatePurchase(
        ...
        currentUser,
        `Purchase marked as Completed by ${currentUser.name || currentUser.email}.`,
        currentUser.accessToken
      );
  ...
  const handleDeletePurchase = async () => {
    // SECURITY NOTE: Client-side role checks are for UI/UX only. Server-side authorization is crucial for security.
    if (!purchaseToDelete || !currentUser || !isSuperAdmin(currentUser.role) || !currentUser.accessToken) {
      toast.error('Unauthorized to delete purchases.');
      return;
    }
    toast.info(`Deleting purchase ${purchaseToDelete.purchaseNo}...`);
    try {
      await apiService.deletePurchase(purchaseToDelete.id, currentUser.accessToken);
        ...
        <div className="flex items-center gap-3">
          ...
          {(isSuperAdmin(currentUser.role) || isPurchaseEntry(currentUser.role) || AuthUtils.canAccess(currentUser, accessConfig)) && (
            <Dialog open={isNewPurchaseDialogOpen} onOpenChange={setIsNewPurchaseDialogOpen}>
              <Button
                className="invisible bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2 transform transition-transform duration-200 hover:scale-105"
                onClick={handleOpenNewPurchase}
              >
                <Plus className="w-5 h-5" />
                <span className="hidden md:inline">New Purchase Entry</span>
              </Button>
              <Button
...
export default function App() {
  ...
  useEffect(() => {
    const validateSession = async () => {
      try {
        ...
        const allowedRoles = ['super-admin', 'purchase-entry', 'operations', 'accountant'];
        if (!allowedRoles.includes(user.role)) {
          toast.error('Access denied. Your role does not permit access to this dashboard.');
          router.replace(`/`);
          return;
        }
        // ✅ Role-based view display logic
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    if (!currentUserProfile?.accessToken) {
      ...
      setPurchaseError('Authentication token missing. Please log in.');
      return;
    }
    try {
      const fetched = await apiService.fetchPurchases(currentUserProfile.accessToken);
      ...
      // If fetching fails due to auth, prompt login
      setCurrentUserProfile(null);
    } finally {
      setLoadingPurchases(false);
    }
  ...
  useEffect(() => {
    ...
    if (storedUser && storedAccessToken) {
      try {
        ...
        user.accessToken = storedAccessToken; // Ensure accessToken is part of the profile
        setCurrentUserProfile(user);

        // Set default view mode based on role on initial load
  ...
  useEffect(() => {
    if (currentUserProfile && currentUserProfile.accessToken) {
      fetchPurchases();
    } else {
      setPurchases([]);
    }
  }, [currentUserProfile, fetchPurchases]);
  ...
  const { pageActions } = createSimplePageConfig(
    currentUserProfile,           // 1. User Profile
    loadingPurchases,             // 2. Loading state
    fetchPurchases,               // 3. Refresh function (Pass the function here!)
    setIsNewPurchaseDialogOpen,   // 4. Dialog state setter
    'New Purchase',               // 5. Button text (optional)
    'bg-blue-600 hover:bg-blue-700' // 6. Button color (optional)
  );

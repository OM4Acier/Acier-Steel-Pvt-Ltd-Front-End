...
export const TaskCard = React.memo(({
  ...
  const canDeleteTask = useMemo(() => canPerform("delete"), [canPerform]);
  const canAssign = useMemo(() => canPerform("update"), [canPerform]);
...
<div
  ...
  className={cn(
    "group relative flex flex-col rounded-xl transition-all duration-300 cursor-pointer overflow-hidden",
...
>
  ...
  <div className="relative px-4 py-3 flex items-center justify-between gap-3 overflow-hidden border-b border-slate-50 dark:border-slate-800">
    ...
    {(isWarning || isCritical) && (
      <div 
        className={cn(
          "absolute -top-10 -right-10 w-32 h-32 blur-3xl transition-all duration-500 opacity-60 pointer-events-none",

  ...
  return (
    ...
    >
      ...
      <CardContent className="p-4 space-y-3 text-gray-800 dark:text-gray-200 dark:bg-gray-800">
        ...
        {order?.customerPaymentStatus === 'new-unpaid' && (
          <div className="flex items-center text-sm font-black text-amber-700 dark:text-amber-400 mt-2 p-3 rounded-xl bg-amber-100 dark:bg-amber-950/40 border-2 border-amber-600 shadow-inner">
            <AlertCircle className="w-6 h-6 mr-3 flex-shrink-0 text-amber-600" />
            <div className="flex flex-col leading-tight">
              <span className="uppercase tracking-tighter text-xs">Unpaid Order</span>
              {userRole === 'operations' && (
                <span className="text-sm font-bold opacity-90">भुगतान लंबित है</span>
              )}
            </div>
          </div>
        )}

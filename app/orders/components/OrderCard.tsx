...
export const OrderCard: React.FC<OrderCardProps> = ({
  ...
  // Apply a distinct, impressive style for high priority orders (Refined Priority Glow)
  const priorityStyle = order.isHighPriority
    ? 'border-2 border-red-500 shadow-xl shadow-red-500/40' // Defined border + soft glow
    : 'border border-gray-300 dark:border-gray-600 hover:shadow-md'; // Standard subtle border

  return (
    <Card
      onClick={() => setSelectedOrder(order)}
      // Card BG is set to dark:bg-gray-900 (slightly darker than content for separation)
      className={`cursor-pointer transition-all duration-300 transform hover:-translate-y-1 rounded-xl overflow-hidden min-w-[280px] bg-white dark:bg-gray-900 ${priorityStyle}`}
    >
      {/* HEADER SECTION: Status (Left) and Badges (Right) */}
      <CardHeader
        className={`p-4 ${
          statusColors[order.status || 'Order Created']

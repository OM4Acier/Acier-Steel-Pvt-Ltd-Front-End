...
export const OrderDetailsDialog: React.FC<OrderDetailsDialogProps> = ({
  ...
  const handleSaveAllChanges = useCallback(async () => {
    if (!currentUserProfile?.accessToken || !currentOrder) {
    ...
    try {
      ...
      if (Object.keys(pendingChanges.textFields).length > 0) {
        ...
        await apiService.updateOrder(currentOrder.deoNo, payload, historyEntry, currentUserProfile.accessToken);
      }
      ...
      const freshOrderData = await apiService.fetchOrders(currentUserProfile.accessToken);
  ...
  const handleDeleteUploadedFile = async (fileId: string, stage: 'product' | 'vehicle' | 'invoice') => {
    ...
    try {
      await apiService.deleteFile(fileId, currentUserProfile.accessToken);

      // Refresh order data after deletion
      const freshOrderData = await apiService.fetchOrder(currentOrder.deoNo, currentUserProfile.accessToken);
  ...
  const handleUpdateOrderStatus = async (newStatus: string) => {
    ...
    if (!apiService.canTransitionToGeneral(currentOrder.status || '', newStatus)) {
    ...
    try {
      await apiService.updateOrderStatus(currentOrder.deoNo, newStatus, historyEntry, currentUserProfile.accessToken);
      onOrderUpdated();
      onClose();
  ...
  const handleMarkAsPaid = async () => {
    if (!currentUserProfile?.accessToken || !currentOrder) return;
    ...
    try {
      await apiService.updateOrder(currentOrder.deoNo, { customerPaymentStatus: 'new-paid' }, historyEntry, currentUserProfile.accessToken);
      onOrderUpdated();
      onClose();
  ...
  const handleCreatePartDeliveryOrder = async () => {
    ...
    try {
      ...
      const createdPartOrder = await apiService.addOrder(newOrder, currentUserProfile.accessToken);
      ...
      await apiService.updateOrder(currentOrder.deoNo, {}, historyEntry, currentUserProfile.accessToken);
        ...
        <DialogFooter className="mt-6 flex flex-wrap justify-start gap-3 border-t border-gray-200 dark:border-gray-700 pt-4">
          {isEditMode ? (
            ...
            <>
              ...
              {apiService.canTransitionToGeneral(displayOrder?.status || '', 'Approved for Production') &&
              ...
              {apiService.canTransitionToGeneral(displayOrder?.status || '', 'Ready for Dispatch') &&
              ...
              {apiService.canTransitionToGeneral(displayOrder?.status || "", "Dispatched and Invoiced") &&
                canEditOperationsSpecificFields(currentUserProfile?.role ?? null) && (
              ...
              {apiService.canTransitionToGeneral(displayOrder?.status || '', 'Completed') &&
                  ...
                  <Dialog open={isMoreActionsOpen} onOpenChange={setIsMoreActionsOpen}>
                    ...
                    <DialogContent className="sm:max-w-[350px] md:max-w-[550px] p-4 bg-white dark:bg-gray-800 rounded-xl shadow-lg z-[9001]">
                      ...
                      <div className="flex flex-col gap-3 mt-4">
                        ...
                        {apiService.canTransitionToGeneral(displayOrder?.status || '', 'Cancelled') &&
